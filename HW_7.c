...
void load_program(struct PCB *program);
char output[2];
char input_line[9];
int fp, ret;
short int pc = 0;
...
int main(int argc, char* argv[]) {
    ...
    struct PCB *head = Program1;
    struct PCB *tail = Program2;
    Program1 -> PID = 0;
    Program1 -> BaseReg = 0;
    Program1 -> LimitReg = 99;
    Program1 -> Next_PCB = Program2;
    Program1 -> Prev_PCB = Program2;
    ...
    }//Sucessfully reaches here

    //Current Problem:
    //It loops 5-10 because it does save into program 2 correctly but doesn't reassign to tmp correctly
    //sp changes from 100 to 0 on program 2 that is the problem need to figure out how it is doing it
    ...
    while(1){
        ...
        while(1){
            if(opcode(memory[program_line+sp]) == 99){
                ...
                if(sp == 0){//If program 1 save to program 1
                    save_program(Program1);
                }else if(sp == 100){//If program 2 save to program 2
            ...
            if(program_line % tmp->context_switch == 0){
                if(sp == 0){
                    save_program(Program1);
                }else if(sp == 100){
                    save_program(Program2);
        ...
        if(head == tail && end_flag == 1){
            printf("Done all programs\n");
            break;
        }
        if(tmp == tail && end_flag == 1){
            printf("Delete Tail\n");
            
            tail = tmp->Prev_PCB;//Assign tail node to the 2nd to last node
            tail->Next_PCB = head;
            head->Prev_PCB = tail;
            free(tmp);
            tmp = head;
            ...
            sp = tmp->BaseReg;
            continue;
        }else if(tmp == head && end_flag == 1){
            printf("Delete Head\n");
            head = tmp->Next_PCB;//Head is the next PCB node
            tail->Next_PCB = head;
            head-> Prev_PCB = tail;
            free(tmp);
            tmp = head;
...
void load_program(struct PCB *program){
    ...
    p3 = program->p3;
    PID = program -> PID; 
    strncpy(psw, program->psw, strlen(psw));
    program_line = program->program_line;
    acc = program->acc;
